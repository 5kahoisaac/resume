...
interface Props {
  ...
  onReset$: QRL<() => void>;
  onAccentChange$: QRL<(hex: string) => void>;
  accent: string;
}

/**
 * The sticky top toolbar. Houses all "global" actions — exports, imports,
 * theme, reset, zoom. The file input is hidden and triggered programmatically
 * so we get a tidy custom button instead of the native file widget.
 */
export const Toolbar = component$<Props>((props) => {
  const fileInputRef = useSignal<HTMLInputElement>();

  // Curated swatches for the accent picker — pulled from common resume palettes
  const swatches = ["#E67E22", "#1F3A5F", "#0F766E", "#7C3AED", "#BE185D", "#111827"];
      ...
      <div class="flex items-center gap-3 px-5 py-3">
        ...
        <div class="flex items-center gap-2 text-brand-slate">
          ...
          <input
            ...
            onInput$={(_, el) => (props.zoom.value = Number(el.value))}
            class="accent-brand-orange w-28"
          />
        ...
        <div class="flex items-center gap-2">
          <span class="font-mono text-[11px] uppercase tracking-wider text-brand-slate">Accent</span>
          {swatches.map((c) => (
            <button
              key={c}
              type="button"
              title={c}
              onClick$={() => props.onAccentChange$(c)}
              class={[
                "h-5 w-5 rounded-full transition-transform hover:scale-110",
                props.accent === c ? "ring-2 ring-offset-2 ring-brand-navy" : "",
              ].join(" ")}
              style={{ background: c }}
...
// ---------------------------------------------------------------------------
// ToolbarButton — same shape across variants. `loading` disables interaction
// without changing the visual width (so the bar doesn't jump).
// ---------------------------------------------------------------------------
